function download_and_collate_mimiciii_ppg_af_dataset
% DOWNLOAD_AND_COLLATE_MIMICIII_PPG_AF_DATASET  Downloads and collates data from 
% the MIMIC-III Waveform Database Matched Subset, for PPG beat detector performance
% evaluation.
%   
%  Inputs:
%
%    files    -  none (although see 'setup_up' for the parameters to be set)
%
%  Working Files:
%
...
%  Outputs:
%
%    file     -  Two MATLAB files containing all the required data for PPG beat
%               detector evaluation: 'mimic_af_data.mat', and 'mimic_non_af_data.mat'.
%
%  Preparation:
%
...
% Settings
up.settings.req_durn = 1*60*60; % minimum duration of recording (in secs)
...
%         end
    ...
    for file_no = 1 : length(possible_files)
        ...
        if file_no == 1 || (subj_no == 35 && file_no == 2)
    ...
    req_samps = 1 + (up.settings.req_durn*125); 
    for sig_no = 1 : length(sigs)
        eval(['rel_data = data(inc_subj_no).' sigs{sig_no} ';']);
        if ~isempty(rel_data)
            eval(['data(inc_subj_no).' sigs{sig_no} '.v = data(inc_subj_no).' sigs{sig_no} '.v(1:req_samps);']);
        end
    end
